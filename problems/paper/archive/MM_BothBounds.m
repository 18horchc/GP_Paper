% Paper figure: Michaelis-Menten GP with Pensoneault lower + upper bounds.
% Baseline SE-GP vs fmincon fit with:
%   mu_f(x_c) - k*sigma_f(x_c) >= 0
%   mu_f(x_c) + k*sigma_f(x_c) <= Vmax
% at 41 grid points. No data-fidelity tube.
...
%% Pensoneault-constrained GP (lower at 0 + upper at Vmax; sigma_n fixed)
...
nonlcon = @(theta) pens_constraints_both(theta, hyp_tpl, inffunc, meanfunc, covfunc, likfunc, ...
...
fprintf('\n=== Pensoneault GP (lower bound at 0 + upper bound at Vmax) ===\n');
...
figure('Color', 'w', 'Position', [80, 80, 1100, 520], ...
    'Name', 'Michaelis-Menten GP: both bounds');
tiledlayout(1, 2, 'Padding', 'compact', 'TileSpacing', 'compact');

panels(1) = struct('m', m_unc, 'sf', sf_unc, 'title', 'Baseline GP');
panels(2) = struct('m', m_con, 'sf', sf_con, 'title', 'Bounded GP');
...
function [c, ceq] = pens_constraints_both(theta, hyp_tpl, inffunc, meanfunc, covfunc, likfunc, ...
    x, y, X_c, k, y_max)
% Pensoneault-style lower + upper bounds on latent f at X_c.
% lower: mu - k*sigma >= 0  <=>  c_lower <= 0
% upper: mu + k*sigma <= y_max  <=>  c_upper <= 0
