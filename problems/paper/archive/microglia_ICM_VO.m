% microglia_ICM_VO.m — Naive SE-GP vs ICM MOGP, with and without a virtual
% observation at (t=0, y=5, sigma=0.01) on both M1 and M2.
% Datasets: full and averaged raw counts. No Pensoneault bounds.
%
% Figure 1: naive SE vs ICM SE (coregionalization B).
% Figure 2: same comparison with heteroscedastic VO (real sn from unconstrained
%         fit; VO uses sigma=0.01 in count units).
...
fprintf('=== Microglia: naive SE vs ICM SE (+ VO) ===\n');
fprintf('kernel = %s | VO: t=%.3g, y=%.3g, sigma=%.3g\n', ...
    kernel_name, t_vo, y_vo, sigma_vo);
...
%% ===== Fit naive + ICM (+ VO) for each dataset =====
results = struct([]);
for didx = 1:numel(datasets)
    ...
    results(didx).naive_vo = naive_vo;
    ...
    report_fit(ds.name, 'Naive GP + VO', naive_vo.report);
    report_fit(ds.name, 'ICM MOGP + VO', icm_vo.report);
end

%% ===== Figure 1: naive vs ICM (no VO) =====
...
%% ===== Figure 2: naive vs ICM with VO =====
figure(2);
set(gcf, 'Color', 'w', 'Position', [100, 40, 1240, 900], ...
    'Name', 'Microglia: naive SE vs ICM SE with VO at (0,5)');
tiledlayout(2, 2, 'Padding', 'compact', 'TileSpacing', 'compact');

for didx = 1:numel(results)
    ds = results(didx);
    methods = {ds.naive_vo, ds.icm_vo};
    method_titles = {'Naive GP + VO', 'ICM MOGP + VO'};
    for midx = 1:2
        nexttile;
        ax = gca; ax.Layer = 'top';
        hold on; grid on;
...
out.report.B = B;
out.report.rho = corr_from_B(B);
out.report.sn = sn_real;   % fixed real-data noise (std units)
end
...
% Independent SE GPs with heteroscedastic VO via gp_seiso_hetero_noise.
